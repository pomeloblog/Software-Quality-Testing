...
RESULT_TEXT = (
    "分模块执行结果如下（12 条用例全部自动化执行，断言失败即 NG）："
)

RESULT_TBL = [
    ["模块", "用例数", "通过", "失败", "通过率", "失败用例"],
    ["M3 图书浏览与检索", "6", "2", "4", "33.3%", "BS-IT-042/044/045/046"],
    ["M6 后台管理", "6", "3", "3", "50.0%", "BS-IT-102/104/105"],
    ["合计", "12", "5", "7", "41.7%", "对应 8 个有效缺陷"],
]

DEFECT_TBL = [
    ["编号", "缺陷简述", "模块", "严重度", "发现方式", "状态"],
    ["BUG-M3-01", "pageSize=0/-1 服务端500（除零/SQL错误）", "M3", "高", "自动化", "打开"],
    ["BUG-M3-02", "搜索翻页链接未URL编码，条件丢失", "M3", "高", "自动化", "打开"],
    ["BUG-M3-03", "价格区间 min>max 无校验无提示", "M3", "中", "自动化", "打开"],
    ["BUG-M3-04", "价格参数仅支持整数，小数静默失效", "M3", "中", "自动化", "打开"],
    ["BUG-M3-05", "销量榜单 LIMIT 1,50 跳过第一名", "M3", "中", "自动化", "打开"],
    ["BUG-M6-01", "新增图书无校验：空名/负价/NULL价入库", "M6", "高", "自动化", "打开"],
    ["BUG-M6-02", "删除图书非数字id 500；不存在id静默", "M6", "中", "自动化", "打开"],
    ["BUG-M6-03", "新增重复用户名唯一约束冲突 500", "M6", "高", "自动化", "打开"],
]

ROOT_CAUSE = (
    "分析一（BUG-M3-01，pageSize=0/-1 服务端 500，高/健壮性）："
    "①发现：BS-IT-042 以 pageSize=0 请求分页返回 HTTP 500；"
    "②定位：BookServiceImpl.page() 第 59 行 pageTotalCount % pageSize 抛除零 ArithmeticException；"
    "pageSize=-1 时 pageTotal 为负、pageNo 归一为负，LIMIT 起始/条数为负触发 MySQL 语法错误；"
    "③根因：parseInt 已容错非数字，但解析结果未做下界校验（<1 时取默认值）；"
    "④修复建议：Service 层加 if (pageSize < 1) pageSize = Page.PAGE_SIZE;；"
    "⑤验证方案：修复后 pageSize=0/-1/abc 均返回 200 且每页 4 条，run-tests.bat 复测 test042 应通过。\n"
    "分析二（BUG-M6-01，新增图书无输入校验，高/数据正确性）："
    ...
    "④修复建议：add/update 入口校验 name 非空、price≥0 且合法数字、sales/stock≥0，不合法转发回编辑页并提示；"
    "⑤验证方案：三类非法输入均被拒绝且有提示，run-tests.bat 复测 test102 应通过。\n"
    "分析三（BUG-M3-05，榜单跳过第一名，中/功能正确性）："
    ...
    "④修复建议：改为 LIMIT 50（一行改动）；"
    "⑤验证方案：修复后重跑 run-tests.bat，test046 应通过且榜首为销量最高图书。"
)

QUALITY = (
    "M3 图书浏览与检索、M6 后台管理共 12 条用例，通过 5 条（41.7%），确认有效缺陷 8 个（高 4 / 中 4），全部处于打开状态。\n"
    "从缺陷分布看，正常功能路径（分页浏览、搜索命中、合法数据增删改、账单统计、发货流转）实现正确——"
    "总账单四项统计与直查数据库完全一致、发货状态流转正常；但异常输入防御普遍薄弱：8 个缺陷中 6 个属于"
    "\"参数校验缺失\"（pageSize 下界、空书名、负价、min>max、非数字 id、重复用户名），2 个属于实现细节错误"
    "（URL 未编码、LIMIT 偏移）。\n"
    "结论：两个模块未达发布标准。建议优先修复 4 个高级别缺陷（其中 BUG-M3-05 仅需一行改动），"
    "修复后以 tests/run-tests.bat 对 12 条用例做回归验证，预期通过率可恢复至 100%。"
)
...
def build_report1():
    ...
    # 5.2 缺陷摘要
    p = replace_body_text(doc, "针对每个缺陷，简要描述", "初测共确认有效缺陷 8 个（高 4 / 中 4），全部由自动化用例执行发现，均处于打开状态（附修复建议与验证方案）。摘要如下：")
